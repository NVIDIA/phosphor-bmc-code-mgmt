...
class Download : public DownloadInherit
{
  public:
     ...
     *<key>").
     **/
    void addRemoteServerPublicKey(std::string serverAddress,
                                  std::string publicKeyStr) override;
     ...
     * @param[in] serverAddress  - The server IP address.
     **/
    void revokeAllRemoteServerPublicKeys(std::string serverAddress) override;
  ...
  private:
     ...
     **/
    inline void updateDownloadStatusProperties(
        std::string& sourceFile, std::string& destDir, DownloadStatus status)
    {
