...
PrisAPFWStateMachine::PrisAPFWStateMachine(MachineContext& ctx) :
    ...
    fwUpdateManager(
        std::any_cast<UpdateManager*>(ctx.GetData(keyUpdateManager)))
{
    deviceLayer =
        std::make_unique<I2CCommLib>(PrisAPFWStateMachine::busIdentifier,
                                     PrisAPFWStateMachine::deviceAddrress);

    maxtimerCounter = 0;

    stateFlowSequence = {
        &FuncIdle,      &FuncCheckCECStatus,    &FuncStartFwUpdate,
        &FuncCopyImage, &FuncCheckUpdateStatus, &FuncTerminate};
}
...
void PrisAPFWStateMachine::StateCheckCECStatus(
        ...
        if (retVal != static_cast<uint8_t>(I2CCommLib::CommandStatus::SUCCESS))
        {
            ...
            else
            {
                log<level::ERR>("APFW:StateCheckCECStatus - CEC FW OTHER ERR.",
                                entry("ERR=0x%x", retVal));
                msg = "CEC Update Status: " +
                      deviceLayer->GetCommandStatusStr(retVal);
            }
        }
        auto updateManager = std::any_cast<UpdateManager*>(
            fwUpdateMachine->myMachineContext.GetData(keyUpdateManager));
...
void PrisAPFWStateMachine::StateStartFwUpdate(MachineContext& ctx)
{
    ...
    try
    {
        ...
        uint32_t fileSize = std::any_cast<uint32_t>(ctx.GetData(keyFWFileSize));
        ...
        {
            throw std::runtime_error("APFW:StateStartFwUpdate: Bad image");
        }
        ...
        {
            std::string tmpErr{
                "APFW:StateStartFwUpdate: Invalid image, file: "};
            tmpErr += imgFileName;
            throw std::runtime_error(tmpErr.c_str());
        }
        ...
        imgFileSize +=
            static_cast<uint32_t>(imageData[I2CCommLib::OTA_OFFSET_SIZE1]);
        imgFileSize +=
            static_cast<uint32_t>(imageData[I2CCommLib::OTA_OFFSET_SIZE2]) << 8;
        imgFileSize +=
            static_cast<uint32_t>(imageData[I2CCommLib::OTA_OFFSET_SIZE3])
            << 16;
        imgFileSize +=
            static_cast<uint32_t>(imageData[I2CCommLib::OTA_OFFSET_SIZE4])
            << 24;
...
void PrisAPFWStateMachine::StateCopyImage(MachineContext& ctx)
{
    ...
    try
    {
        if (!fwUpdateManager)
        {
            ...
            throw std::runtime_error(
                "APFW:StateCopyImage: Failed in state transition");
        }
        ...
        std::string copyImageServiceFile =
            std::string("nvidia-secure-block-copy@\\x2D") + "f\\x20" +
            imgFileName + "\\x20\\x2Ds\\x20" + std::to_string(imgFileSize) +
            ".service";
...
void PrisAPFWStateMachine::StateCheckUpdateStatus(
    [[maybe_unused]] MachineContext& ctx)
{
    bool stateSuccessful{true};
    ...
    try
    {
        ...
        else
        {
                    ...
                    I2CCommLib::FirmwareUpdateStatus::STATUS_UPDATE_FINISH))
            {
                stateSuccessful = false;
                log<level::ERR>(
                    "APFW:StateCheckUpdateStatus - Firmware update failed.",
                    entry("ERR=0x%x", retVal));
                msg =
                    "CEC Update Check: Firmware update failed with status: update didn't finished (" +
...
void PrisAPFWStateMachine::StateTerminate([[maybe_unused]] MachineContext& ctx)
{
    ...
    else
    {
        ...
        {
            log<level::DEBUG>(
                "APFW:StateTerminate - Firmware update succeeded.");
        }
        catch (const std::exception& e)
        {
            msg += e.what();
    ...
    catch (const std::exception& e)
    {
        log<level::ERR>("APFW:StateTerminate - Failed in state transition.",
                        entry("EXCEPTION=%s", e.what()));
        throw std::runtime_error(
            "APFW:StateTerminate: Failed in state transition");
    }
}
