...
int main(int argc, char** argv)
{
    ...
    app.add_option("-f", publicKeyFilename, "Public key filename to be used for signature validation.");
    app.add_option("-n", randomNumberStr, "32 byte random number in hex format to be used as challenge.");
    ...
    try
    {
        ...
        fsys::path pKeyFileDestination(cecAttestFolder + cecAttestPublicKeyFile);

        if(fsys::exists(publicKeyFile.string()) && is_regular_file(publicKeyFile))
        {
        ...
        if(randomNumberStr.size())
        {
            ...
            std::ofstream randomNumberFile(randFile.string(),std::ios::binary | std::ios::out);
            randomNumberFile.write(reinterpret_cast<char*>(&randomNumberStr[0]),randomNumberStr.size());
    ...
    {
        log<level::ERR>(
            "nvidia firmware service - GetAttestation failed.",
            entry("EXCEPTION=%s", e.what()));
